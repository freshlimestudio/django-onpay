...
"""\
Основной модуль, ядро, работающее с onpay

Не привязан к конкретному фреймоворку, может использоваться для привязки
к фреймворку, отличному от django.

Для привязки к фреймоворку потребуется перенастроить модуль conf и написать
вьюхи/контроллеры, которые по понятным причинам должны соответствовать
другим частям сайта. Можно сделать на основе django-views (views.py).

Также потребуется место для хранения информации о заявках на пополнение счета
и прочей оперативной информации, примеры для django есть в файле models.py.
"""
